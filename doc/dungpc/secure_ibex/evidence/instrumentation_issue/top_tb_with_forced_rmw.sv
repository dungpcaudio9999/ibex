...
module top_tb;
  ...
  parameter bit SEC=0, CACHE=0, PRED=0, DUAL=0, MODE=0, SCR=0, ICECC=0, PMP=0;
  parameter int ZC=0, OFFSET=1;
  parameter bit TWEAK=SEC;
  logic clk=0,rst=1;
  always #5 clk=~clk;
  ibex_mubi_t mode,fetch_en;
  assign mode = (fault==5 && cycle>=100) ? ibex_mubi_t'(0) : (MODE ? IbexMuBiOn : IbexMuBiOff);
  ...
  string program_file,wave;
  typedef struct packed {int due; logic [31:0] data; bit tag; bit err;} rsp_t;
  rsp_t iq[$],dq[$],tmp;
  assign ig=ir && grant_open;
  assign dg=dr && grant_open;
  ibex_top #(.SecureIbex(SEC),.WritebackStage(1),.BranchTargetALU(1),
    .BaseIsa(DUAL ? BaseIsaRV32IorCHERIoT : BaseIsaRV32I),.RV32ZC(rv32zc_e'(ZC)),
    .RV32M(RV32MSingleCycle),.RV32B(RV32BFull),.ICache(CACHE),.ICacheECC(ICECC),
    .ICacheScramble(SCR),.ICacheTweakInfection(TWEAK),.BranchPredictor(PRED),.LockstepOffset(OFFSET),.PMPEnable(PMP),
  ...
  initial begin
    ...
    if($value$plusargs("wave=%s",wave))begin $dumpfile(wave);$dumpvars(0,top_tb);end
    $display("CONFIG SEC=%0d CACHE=%0d PRED=%0d DUAL=%0d MODE=%0d ZC=%0d SCR=%0d ICECC=%0d OFFSET=%0d PMP=%0d",SEC,CACHE,PRED,DUAL,MODE,ZC,SCR,ICECC,OFFSET,PMP);
    #1;rst=0; #21;rst=1;
  end
  always @(negedge clk) begin
    ...
    if(rst)begin
      ...
      if(!fired && ((ev==1 && sleeping && cycle>80) || (ev inside {2,3,4} && dq.size()>0)))begin
        fired=1;
        if(ev==1 || ev==4)irq=1;
        if(ev==2 || ev==4)dbg=1;
        if(ev==3)nmi=1;
  ...
  if(CACHE)begin
    ...
    always @(negedge clk) begin
      if(fault==10 && cycle==350)begin
        saved_cache_word=dut.u_ibex_core.if_stage_i.gen_icache.icache_i.hit_data_ecc_ic1 ^ 1;
        force dut.u_ibex_core.if_stage_i.gen_icache.icache_i.hit_data_ecc_ic1=saved_cache_word;
        $display("INJECT %0d cache-hit-codeword",cycle);
      end
      if(fault==10 && cycle==354)release dut.u_ibex_core.if_stage_i.gen_icache.icache_i.hit_data_ecc_ic1;
    end
  ...
  if(SEC)begin
    ...
    always @(negedge clk) begin
      #1;
      if(rst && cycle<120)$display("SAMPLE %0d ai=%b ab=%b am=%b pc=%b mismatch=%b rf=%b mode=%b",cycle,aint,abus,amin,dut.u_ibex_core.pc_mismatch_alert,dut.gen_lockstep.u_ibex_lockstep.outputs_mismatch,dut.gen_lockstep.u_ibex_lockstep.u_shadow_core.rf_ecc_err_comb,dut.u_ibex_core.cheriot_enable_mubi_err);
    end
  ...
  always @(posedge clk)begin
    if(rst)begin
      ...
      if(ir && ig)begin iq.push_back(rsp_t'{cycle+latency,imem[ia[11:2]],1'b0,ia==32'(iferr)});if_grants++;end
      if(dr && dg)begin
        dq.push_back(rsp_t'{cycle+latency,dmem[da[11:2]],tags[da[11:2]],1'b0});data_grants++;
