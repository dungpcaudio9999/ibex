...
module top_tb;
  ...
  ibex_top #(.SecureIbex(SEC),.WritebackStage(1),.BranchTargetALU(1),
    ...
    .ICacheScramble(SCR),.BranchPredictor(PRED),.LockstepOffset(OFFSET),.PMPEnable(PMP),
  ...
  always @(negedge clk) begin
    ...
    if(rst)begin
      ...
      if(key_wait>0)begin key_wait--;if(key_wait==1)begin keyvalid=1;key=key+1;nonce=nonce+1;end end
  ...
  always @(posedge clk)begin
    ...
    if(cycle==1199)begin
      ...
      $display("COUNTS sleep=%0d clocks=%0d ai=%0d ab=%0d am=%0d ifetch=%0d data=%0d bitmap=%0d hit=%0d miss=%0d dummy=%0d expanded=%0d predict=%0d mispredict=%0d",sleep_cycles,clock_edges,alerts_i,alerts_b,alerts_m,if_grants,data_grants,bm_grants,cache_hits,cache_misses,dummies,expanded,predicts,mispredicts);
