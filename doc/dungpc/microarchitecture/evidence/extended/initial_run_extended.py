...
CONFIGS={'top':{},'predict':{'PRED':1},'zc':{'ZC':3},'cache':{'CACHE':1},
         ...
         'secure_dual':{'SEC':1,'DUAL':1,'MODE':1},'pmp':{'PMP':1}}
...
def build(config,out):
    ...
    cmd += [f'-G{k}={v}' for k,v in CONFIGS[config].items()]+[str(p) for p in sources]+[str(HERE/'top_tb.sv')]
...
def main():
    ...
    for config in opts.configs.split(','):
        ...
        for case in cases:
            ...
            if not case.startswith('fault') and any(counts.get(k,0) for k in ('ai','ab','am')):errors.append('unexpected alert')
